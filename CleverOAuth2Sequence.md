...
- `redirectUri`:
  ...
  - e.g., https://clusive.cast.org/accounts/clever/
...
?response_type=code
&redirect_uri=https%3A%2F%clusive.cast.org/accounts/clever uri registered at (I)
...
```
GET
https://clusive.cast.org/accounts/clever/
...
Content-Length: Length of the json below.
{
    ...
    "redirect_uri": "https://clusive.cast.org/accounts/clever/" `redirectUri` from (I)
}
```
...
https://clever.com/oauth/authorize
?response_type=code
&redirect_uri=https%3A%2F%clusive.cast.org/accounts/clever  uri used by Clever's request above.
&client_id=`clientId` the `clientId` registered at (I)
