...
- `redirectUri`:
  ...
  - e.g., https://clusive.cast.org/accounts/clever/login/callback/
  - e.g., http://localhost:8000/accounts/clever/login/callback/  (for testing)
  - a list of uris is allowed
- `scope`:
...
Note: the following uses the django-allauth paradigm.  The actual details may be
different:
...
Also, Clusive is allowed to specify a different `redirectUri` in the request
...
redirect.  Maybe there is one, but it seems less secure than a uri prearranged 
...
?response_type=code
&redirect_uri=https%3A%2F%clusive.cast.org/accounts/clever/login/callback/ uri registered at (I)
&client_id=`clientId` the `clientId` registered at (I)\
&state=random characters generated by Clusive to be echoed back by Clever.
```
Note that the use of an `/authorize` endpoint complies with the [OAuth2
specification](https://tools.ietf.org/html/rfc6749#section-4.1.1).  The
specification recommends the use of the `state` parameter as a way of protecting
against cross-site forgeries.
...
```
GET
https://clusive.cast.org/accounts/clever/login/callback/
...
passed by Clusive at step 1.  A guess is Clusive generates and sends it as a 
paremeter in the initial request to Clever's SSO page*.

Clusive will need to keep track of the `code` value and retrieve the `clientId`
and `clientSecret` from its database, specifically from its Clever Social
application record.
...
Content-Length: Length of the json below.
{
    ...
    "redirect_uri": "https://clusive.cast.org/accounts/clever/login/callback/" `redirectUri` from (I)
}
```
...
Id, district Id and user type via Clever's API:
```
GET
...
The [Understanding Entry Points](https://dev.clever.com/docs/classroom-getting-started#understanding-entry-points)
section of [Getting Started in the Library](https://dev.clever.com/docs/classroom-getting-started)
lists another signin/authorization possibility that Clusive must support.
Teachers can click "install" from the library (*not clear what that means*) and
Clever will use the first `redirectUri` from the registration (I), passing an
authorization code to Clusive, but not passing a `state` code, as it does at
with the "Sign On with Clever" link sequence, specifically step 3.  *The
following is my Best guess as to what Clever's request looks like:*
```
GET
https://clusive.cast.org/accounts/clever/login/callback/
?`code`=random characters generated by Clever when making this request (one-time)
&`scope`=scope value agreed to during registration (I) *needed in this scenario?*
```

In responding, Clusive is required to generate the `state` code, redirect
to Clever's /oauth/authorize/ end point.  From Clever's documentation
[Protecting against Cross-Site Request Forgery (CSRF)](https://dev.clever.com/docs/il-design#section-protecting-against-cross-site-request-forgery-csrf)
the `state` is generated by Clusive AND associated with the Clever user who is
"installing" Clusive.  A possibility is to store the `state` in the session.
The definition of the `state` is given in the [OAuth2 specification](https://tools.ietf.org/html/rfc6749#section-4.1.1),
and is a set of random characters.  The `state` will be echoed back to Clusive 
when Clever responds to this request; see step 3.  *As noted at step 3, Clever
does not specify how to pass the `state`.  The following is a guess based on
the [OAuht2 specification](https://tools.ietf.org/html/rfc6749#section-4.1.1)*:
```
GET 
https://clever.com/oauth/authorize
?response_type=code
&redirect_uri=https%3A%2F%clusive.cast.org/accounts/clever/login/callback/  uri used by Clever's request above.
&client_id=`clientId` the `clientId` registered at (I)
&state="random characters" the state generated by Clusvie to be echoed back in Clever's response.
```

From here, follow from step 4, on.

## References:
- [Library SSO with OAuth 2.0](https://dev.clever.com/docs/classroom-with-oauth)
- [Getting Started in the Library](https://dev.clever.com/docs/classroom-getting-started)
